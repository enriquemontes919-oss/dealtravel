"""
agents/viajes.py — Agente de viajes vía Awin: Trivago, Kiwi, Sirenis, Expedia, Hoteles.com
"""
from agents.base import (
    AWIN_ID, EXPEDIA_LINK, HOTELES_LINK,
    precio_original, generar_fechas_viaje, ahora_str
)
import requests
...
def run_trivago():
    ...
    for i, (destino, precio, descuento) in enumerate(TRIVAGO_DESTINOS):
        ...
        ofertas.append({
            ...
            "url":             (
                f"https://www.awin1.com/cread.php?s=3330897&v=20563&q=474350&r={AWIN_ID}"
                f"&ued=https://www.trivago.com.mx/?search/200-{requests.utils.quote(destino)}"
            ),
...
def run_sirenis():
    ...
    for i, (nombre, precio, descuento) in enumerate(SIRENIS_HOTELES):
        ...
        ofertas.append({
            ...
            "url":             (
                f"https://www.awin1.com/cread.php?s=3330897&v=20563&q=474350&r={AWIN_ID}"
                f"&ued=https://www.sirenishotels.com"
            ),
            "tipo_promo":      f"-{descuento}% · {fechas}",
