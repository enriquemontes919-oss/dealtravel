...
def clave_oferta(oferta):
    """Genera clave única por destino+fuente para evitar duplicados en alertas"""
    ...
    try:
        ...
        for alerta in alertas:
            try:
                ...
                # 2. Claves destino|fuente ya notificadas (en lugar de IDs)
                ...
                for oferta in ofertas_supabase:
                    # Verificar por clave destino|fuente en lugar de ID
                ...
                if not matches_nuevos:
                    print(f"[Alertas] Alerta {alerta['id']} — sin ofertas nuevas, no se envía")
                    continue
                ...
                # 5. Enviar WhatsApp si el usuario lo eligió y tiene teléfono
                ...
                # 6. Guardar claves notificadas (destino|fuente) y actualizar ultimo_envio
...
def enviar_whatsapp(alerta, ofertas):
    """Genera link wa.me con mensaje prellenado — el usuario recibe el link por email"""
    telefono = alerta.get("telefono", "").strip()
    if not telefono:
        return
    ...
    if numero.startswith("1") and len(numero) == 11:
        pass  # ya tiene código de país USA
    elif not numero.startswith("52") and len(numero) == 10:
        numero = "52" + numero  # agregar código MX
...
def enviar_email_consolidado(alerta, ofertas, dias_alerta, dias_transcurridos):
    ...
    if telefono:
        ...
        mensaje_wa = "\n".join(lineas_wa)
        wa_link = f"https://wa.me/{numero}?text={requests.utils.quote(mensaje_wa)}"
        wa_section = f"""
    ...
    for oferta in ofertas[:8]:
        ...
        precio_display = '<span style="font-size:1rem;font-weight:700;color:#0071e3;">Precio en Amazon →</span>' if sin_precio else f'<span style="font-size:1.4rem;font-weight:800;color:#0ea5e9;">{oferta["precio_fmt"]}</span>'
        ofertas_html += f"""
